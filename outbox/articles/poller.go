...
func NewLikedArticlesPoller(storage ArticleStorage, pollInterval time.Duration, authority string) (*LikedArticlesPoller, error) {
	...
	log.Info().Any("timeout", timeout.Seconds()).Msg("timeout")
...
func (p *LikedArticlesPoller) sendNewEvents(events []ArticleLikedEvent) error {
	...
	if err != nil {
		log.Error().Err(err).Msg("setting deadline")
	}
	log.Info().Any("deadline", deadline).Msg("deadline set")
...
LOOP:
	for {
		...
		if err != nil {
			...
			ok := errors.As(err, &e)
			if !ok {
				log.Error().Err(err).Msg("got unknown error")
				...
				if e.Timeout() {
					log.Error().Err(err).Msg("consumer timed out")
	...
	if loopErr != nil {
		return fmt.Errorf("failed to ack; last received error: %s", loopErr.Error())
	}
	return nil
}
